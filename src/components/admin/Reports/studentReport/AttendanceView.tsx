...
export const AttendanceView = ({ data }: AttendanceViewProps) => {
	...
	const filteredData = useMemo(() => {
		return data.filter((session) => {
			const sessionDate = format(parseISO(session.startTime), "yyyy-MM-dd");
			const matchesSearch = session.title.toLowerCase().includes(searchQuery.toLowerCase());
			const matchesStartDate = startDate ? sessionDate >= startDate : true;
			const matchesEndDate = endDate ? sessionDate <= endDate : true;
	...
	const chartData = useMemo(() => {
		// Show up to 30 sessions in the chart from the filtered set
		// We maintain the reverse order for the chart (Oldest -> Newest)
		return [...filteredData]
			.slice(0, 30)
			.reverse()
			.map((session) => ({
				name: format(parseISO(session.startTime), "MMM dd"),
				duration: Math.round((session.effectiveDuration || 0) / 60),
				status: session.isPresent ? "Present" : "Absent",
			}));
	}, [filteredData]);
				...
				<div className="p-4 border-b border-neutral-200 dark:border-neutral-700 flex flex-col md:flex-row gap-4 justify-between items-center bg-neutral-50/50 dark:bg-neutral-900/50">
					<div className="relative w-full md:w-64">
						...
						<input
							...
							className="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
						/>
					</div>
					<div className="flex items-center gap-2 w-full md:w-auto">
						<input
							...
							className="px-3 py-2 text-sm bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
						/>
								...
								paginatedData.map((row) => (
									...
									>
										...
										<td className="px-6 py-4 text-neutral-500">
											{row.joinTime ? format(parseISO(row.joinTime), "HH:mm") : "-"}
										</td>
										<td className="px-6 py-4 text-neutral-500">
											{row.leaveTime ? format(parseISO(row.leaveTime), "HH:mm") : "-"}
										</td>
										<td className="px-6 py-4 text-right font-mono text-neutral-600 dark:text-neutral-400">
											{Math.round((row.effectiveDuration || 0) / 60)}m
										</td>
									</tr>
								))
							)}
						...
						<div className="flex items-center gap-2">
							...
							<div className="flex items-center gap-1">
								{Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
									...
									// Simple Logic for now, can be improved for large pages
									return pageNum;
								}).map((pageNum) => (
									<button
										type="button"
